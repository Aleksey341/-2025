...
function generateQRCode() {
    ...
    // Генерируем seed для уникальности (подход B)
    const seed = Math.random().toString(36).substring(2, 8);
    const urlWithSeed = `${WISH_PAGE_URL}?s=${seed}`;

    // Используем Google Charts API для генерации QR (простой способ без библиотек)
    const qrApiUrl = `https://chart.googleapis.com/chart?cht=qr&chs=250x250&chl=${encodeURIComponent(urlWithSeed)}&choe=UTF-8`;
    ...
    // Обновляем QR каждые 3 секунды для разнообразия (подход B)
    // Раскомментируйте, если хотите динамический QR:
    /*
    setInterval(() => {
        const newSeed = Math.random().toString(36).substring(2, 8);
        const newUrl = `${WISH_PAGE_URL}?s=${newSeed}`;
        img.src = `https://chart.googleapis.com/chart?cht=qr&chs=250x250&chl=${encodeURIComponent(newUrl)}&choe=UTF-8`;
    }, 3000);
    */
}
